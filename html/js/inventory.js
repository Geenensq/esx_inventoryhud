...
function secondInventorySetup(items) {
    ...
    $.each(items, function (index, item) {
        ...
        '<div class="item-count">' + count + '</div> <div class="item-name">' + item.label +  ( typeof item.stock !== "undefined" ? '(' + item.stock + ')' : ''  )   + '</div> </div ><div class="item-name-bg"></div></div>');
    ...
    $('#otherInventory').droppable({
        drop: function (event, ui) {
            ...
            if (type === "trunk" && itemInventory === "main") {
                ...
                $.post("http://esx_inventoryhud/PutIntoShop", JSON.stringify({
                    item: itemData,
                    number: parseInt($("#count").val())
                }));
    }
